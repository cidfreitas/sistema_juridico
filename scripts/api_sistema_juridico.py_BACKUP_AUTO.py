...
def dashboard():
    ...
    try:
        ...
        resultado = {
            ...
            'novos_movimentos': cursor.execute("SELECT COUNT(*) as total FROM andamentos WHERE data_scraping >= datetime('now', '-1 day')").fetchone()['total'],
...
@app.route("/api/v1/movimentos/recentes", methods=["GET"])
@require_api_key
def movimentos_recentes():
    """Movimentos dos últimos 3 dias"""
    try:
        dias = request.args.get("dias", 3, type=int)
        ...
        cursor.execute("""
            SELECT 
                proc.cnj,
                proc.parte_principal,
                a.data_andamento,
                a.tipo_andamento,
                a.descricao
            FROM andamentos a
            JOIN processos proc ON a.processo_id = proc.id
            WHERE a.data_scraping >= datetime("now", "-" || ? || " days")
            ORDER BY a.data_scraping DESC
            LIMIT 50
        """, (dias,))
        ...
        db.close()
        return jsonify({"total": len(movimentos), "movimentos": movimentos}), 200
